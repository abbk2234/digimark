  ...
  try {
    ...
    if (res.status == 200) {
      ...
      data.db.slider_big_sale.map((elem) => {
        ...
        <div class="swiper-slide bg-body-tertiary rounded border border-red-400 text-center p-1 d-flex flex-column align-items-center" style="height: 250px;">
          <div>
            <img src="${elem.img_url}" class="img-thumbnail border-red-100" alt="" />
          </div>
          <div>
            <p class="txt-5">${elem.name}</p>
            <p class="txt-6 fw-bold"><small>${elem.price}</small>تومان</p>
          </div>
        </div>`;
      });
    } else {
